...
This means I don’t want to debug from my phone. I don’t want to be scrolling through some build pipeline’s output, and study what version of Node.js it’s using, and see what commands it ran and then which one hit a snag. I want to write some Markdown and then it appear online without a hiccup. Ideally GitHub would let me do this, and [they do](https://pages.github.com) but I don’t want to worry about Ruby or Node.js.
...
- When listing the most recent blog posts, this requires a lot of request loading the Markdown for every article to get its title and publication date. This makes loading these index pages a little slow. There’s opportunities for caching here, and I can use the Git SHA to cache things safely forever — for a given SHA, the content is always the same. It’s the power of immutability.
